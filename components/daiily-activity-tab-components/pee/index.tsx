...
const PeeActivity = () => {
    const [peeActivities, setPeeActivities] = useState<any[]>([]);
        ...
        (async () => {
            try {
                ...
                const data: any = await axios.get(
                    `pet/${selectedPetIDForMarketplace}/pee`
                );
                if (Array.isArray(data?.peeActivities)) {
                    setPeeActivities(data.peeActivities);
                }
                setIsLoading(false);
            } catch (e) {
            ...
            <div className="center mt-2">
                {peeActivities.length > 0 ? (
                    <Table responsive striped bordered>
                        ...
                        <tbody>
                            {peeActivities.map((v: any, i: number) => {
                                return (
                                    <tr key={i}>
                                        <td>{v?.pa_date}</td>
                                        <td>{v?.pa_time}</td>
